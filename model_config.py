...
PROVIDER_PARSER_MAPPING = {
    "anthropic": {
        ...
        "stream": lambda chunk: chunk.get("delta", {}).get("text", "") if chunk.get(
            "type") == "content_block_delta" else ""
    },
    "google": {
        "non_stream": lambda data: data["candidates"][0]["content"]["parts"][0]["text"],
        "stream": lambda chunk: next(
            (part.get("text", "") for candidate in chunk.get("candidates", [])
    ...
    "openai": {
        ...
        "stream": lambda chunk: chunk.get("choices", [{}])[0].get("delta", {}).get("content", "")
    },
    "zhipu": {
        "non_stream": lambda data: data["choices"][0]["message"]["content"],
        "stream": lambda chunk: chunk.get("choices", [{}])[0].get("delta", {}).get("content", "")
